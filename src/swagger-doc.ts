...
import { ISwaggerUtils, defaultUtils } from "./default-utils";
...
export class SwaggerDoc extends ObjectEx<PrivateProps> implements ISwaggerDoc {
  ...
  public constructor(
    ...
    if (source.paths) {
      this.paths = Object.keys(source.paths).reduce(
        (accum: SwaggerPath[], key) => {
    ...
    if (source.definitions) {
      this.models = Object.keys(source.definitions).reduce(
  ...
  public addEnums() {
    ...
    const uniqueEnums = uniqueItems(
      this.enums,
      (e) => e.name + JSON.stringify(e.enumValues)
    );
    uniqueEnums.forEach((uniqueEnum) => {
      const similarEnums = this.enums.filter(
        (f) =>
          JSON.stringify(f.enumValues) === JSON.stringify(uniqueEnum.enumValues)
      );
      const newNamespace = similarEnums
        .map((m) => m.namespace)
        .filter((f) => f);
      uniqueEnum.duplicateNamespaces.push(...(newNamespace as any).flat());
      uniqueEnum.duplicateNamespaces = uniqueItems(
        uniqueEnum.duplicateNamespaces,
        (k) => k
      );
