...
export class SwaggerMethod extends SwaggerBase<SwaggerClass, PrivateProps> {
  ...
  public constructor(
    ...
    if (source.requestBody && source.requestBody?.content) {
      const parameter = new SwaggerMethodParameter(
        this,
        source.requestBody?.content["application/json"]
      );
    ...
    if (
      ...
      if (schema) {
        ...
        this.responseModelType.arrayItemType =
          this.utils.getArrayItemType(schema);
        this.responseModelType.type = this.utils.getMethodResponseType(
          this,
          schema
        );
      }
    ...
    if (this.responseModelType.type) {
      this.responseModelType.isJsType = this.utils.isJsType(
        this.responseModelType.type
      );
    }
    this.responseModelType.isEnum = this.utils.isEnum(source)
      ? true
      : undefined;
    ...
    this.isFileUpload = this.parameters.some(
      (s) => s.modelType.type === "File"
    );
    if (source.summary) {
    ...
    if (!this.responseIsVoid) {
      if (!this.responseModelType.isJsType && !this.responseModelType.isEnum) {
        const modelRef = this.doc.models.find(
          (modelItem) => modelItem.name === this.responseModelType.type
        );
        if (modelRef) {
      ...
      if (this.responseModelType.isEnum) {
        const enumRef = this.doc.enums.find((enumItem) =>
          enumItem.keys.includes(this.name)
        );
        if (enumRef) {
          this.responseModelType.enumRef = enumRef;
        } else {
          console.error(
            "Model for enum not found (response method)" + this.name
          );
        }
      }
    }
