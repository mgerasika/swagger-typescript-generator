...
export const SwaggerModelContent: React.FC<IProps> = (props) => {
    const fields = props.swaggerModel.properties.map((parameter: SwaggerModelProperty) => {
        ...
        return (<React.Fragment key={parameter.name}>
            {props.swaggerModel.doc.config.showComments && parameter.description && parameter.description !== parameter.name && <span>{'\t'}<Comment commment={parameter.description} />{'\n'}</span>}
    ...
    return (
        <>
            export declare class {props.swaggerModel.name}{' {\n'}
            {fields}
            {'}'}
        </>
    );
