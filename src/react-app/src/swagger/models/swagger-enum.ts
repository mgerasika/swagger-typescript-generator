...
export class SwaggerEnum
  extends SwaggerBase<SwaggerDoc, PrivateProps>
  implements ISwaggerEnum
{
  public name: string = "";
  ...
  public constructor(
    ...
    if (!this.enumValues && model.methodPropertyDef) {
      this.enumValues = model.methodPropertyDef.modelType.enumValues
        ? model.methodPropertyDef.modelType.enumValues
        : [];
    }
    if (!this.enumValues && model.modelPropDef) {
      this.enumValues = model.modelPropDef.modelType.enumValues
