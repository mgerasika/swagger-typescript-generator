...
import {
  defaultComponents,
  ISwaggerComponents,
} from "../swagger/common/default-components";
...
export const SwaggerDemoComponent: React.FC<IProps> = (props) => {
  ...
  const [state, setState] = useState<IState>({
    ...
    selectedPanelTitle:
      window.localStorage.getItem("selectedPanelTitle") || "Api",
  ...
  const createComponentsFactory = (name: string) => {
    ...
    const result = props.createComponentsFactory
      ? createCustomizationComponentsFactory(
          props.createComponentsFactory(defaultComponents),
          name,
          callback
        )
  ...
  const selectedDefinitionObject = useMemo(() => {
    ...
    return root
      ? root.models.filter((c) => c.name === state.selectedDefinition)
      : [];
  ...
  const selectedEnumObject = useMemo(() => {
    ...
    return root
      ? root.enums.filter((c) => c.getFullName === state.selectedEnum)
      : [];
    ...
    return (
      <div className="row mb-2" style={{ width: "99%" }}>
        <div className="col-sm-12 col-md-5 col-lg-3">
          <pre
            style={{ textAlign: "left", lineHeight: "28px" }}
            className="m-0 p-0"
          >
      ...
      <>
        <BootstrapSelect
          ...
          onChange={(ev) => {
            ...
            if (root) {
              let newRoot = new SwaggerDoc(
                root.config,
                root.utils,
                createComponentsFactory(ev.target.value)
              );
              if (props.createDocumentFactory) {
        ...
        {state.selectedCustomizationMethodName && (
          <textarea
            ...
            onChange={(ev) => {
              const customizationOption = customizationArray.find(
                (f) => f.methodName === state.selectedCustomizationMethodName
              );
    ...
    return root ? (
      <>
        <BootstrapPanel
          ...
          renderSettings={() => (
            <div className="row">
              ...
              <div className="col-md-4 col-sm-12">{renderCustomization()}</div>
            </div>
          )}
          renderContent={() => (
            <DemoAllClassesComponent classes={selectedApiObject} />
          )}
        />
        ...
        <BootstrapPanel
          ...
          renderContent={() => (
            <DemoAllEnumsComponent enums={selectedEnumObject} />
          )}
        />
        ...
        <BootstrapPanel
          ...
          renderContent={() => (
            <DemoAllPathComponent paths={selectedPathsObject} />
          )}
        />
        ...
        <BootstrapPanel
          ...
          renderContent={() => (
            <DiffSingle
              obj={
                <SwaggerAllEnumsExportAdapter doc={root} enums={root.enums} />
              }
            />
          )}
          activeTitle={state.selectedPanelTitle}
        ...
        <BootstrapPanel
          ...
          renderContent={() => (
            <DiffSingle
              obj={
                <SwaggerAllUrlsComponent2 doc={root} classes={root.classes} />
              }
            />
          )}
          activeTitle={state.selectedPanelTitle}
